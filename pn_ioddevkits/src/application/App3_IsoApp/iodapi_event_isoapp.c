...
#include "usrapp_cfg.h"   // example application spec. configuration settings
	...
	)
    {
        ...
        switch (RecordIndex)
        {
            ...
            case 0x8030: // **** IM 4 *****
            {
                REC8030_ISOMDATA data_buffer; 
                *pBufLen = sizeof(REC8030_ISOMDATA); 

                OsMemSet(&data_buffer, 0, sizeof(REC8030_ISOMDATA));
                
                data_buffer.BlockHeader.Type    = OsHtons(0x0204);
                data_buffer.BlockHeader.Len     = OsHtons(*pBufLen - sizeof(data_buffer.BlockHeader) + 2);
                data_buffer.BlockHeader.Version = OsHtons(0x0100);

                data_buffer.SlotNumber          = OsHtons(slot_nr);
                data_buffer.SubSlotNumber       = OsHtons(subslot_nr);
                
                OsMemCpy(pBuffer, &data_buffer, sizeof(REC8030_ISOMDATA));

                PNIO_printf("##READ IsoMData, Api=%d Slot=%d Subslot=%d Len=%d, Sequ_nr=%d\n",
                             Api, slot_nr, subslot_nr, *pBufLen, SequenceNum);
                break;
            }
	        // *----------------------------------------------*
...

                    {
                        ...
                        PNIO_UINT32 TiDelay_ns = 0;
							...
							if ((Slot == 2) && (Status == PNIO_OK))
							{
                                ...
                                if(To_ns == Tcyc_ns)
                                {
                                    Status = PNIO_IsoActivateGpioObj(5, 0,  ISO_GPIO_LOW_ACTIVE, &IsoGpioToHndl);
                                }
                                else
                                {
                        ...
                        PNIO_printf("##WRITE IsoM-Data, Api=%d Slot=%d Subslot=%d Len=%d, Sequ_nr=%d\n",
                                     Api, pAddr->Geo.Slot, pAddr->Geo.Subslot, *pBufLen, SequenceNum);
                                {
                                    PNIO_printf("\n=========== decode ISOM record with index 0x8030 =============\n");
                                    PNIO_printf("Slotnumber          = %10d\n", Slot);
                                    PNIO_printf("SubSlotnumber       = %10d\n", Sub);
                                    PNIO_printf("ConApplCycleFactor  = %10d\n", OsHtons (pIsoM->ControllerApplicationCycleFactor));
                                    PNIO_printf("TimeDataCycle       = %10d nsec\n", Tcyc_ns);
                                    PNIO_printf("TimeIOInput         = %10d nsec\n", Ti_ns);
                                    PNIO_printf("TimeIOOutput        = %10d nsec\n", To_ns);
                                    PNIO_printf("TimeIOInputValid    = %10d nsec\n", OsHtonl (pIsoM->TimeIOInputValid));
                                    PNIO_printf("timeIOOutputValid   = %10d nsec\n", OsHtonl (pIsoM->TimeIOOutputValid));
                                    PNIO_printf("\n");
