...
PNIO_VOID ASSET_PUT8_HTON( PNIO_VOID * base, PNIO_UINT32 offset, PNIO_CHAR val );
...
PNIO_VOID AMR_AddExampleAMRBlocks ( PNIO_VOID )
{
    ...
    const PNIO_CHAR ExampleAnnotation[] = "Insert Description here";
    const PNIO_CHAR ExampleOrderID[] = "xxx-yyyy-zzzz";
    const PNIO_CHAR ExampleSerialNumber[] = "123456789012";

    block1 = AMRData_CreateAMRObject(Asset_FullInformation);
    if (block1 == LSA_NULL)
    {
        PNIO_printf ("ERROR: AMRData_CreateAMRObject() => returned empty block !\n");
        return;
    }
    ...
    AMRObject_IMAnnotation(block1, ExampleAnnotation, sizeof (ExampleAnnotation) - 1);
    AMRObject_SetIMOrderID(block1, ExampleOrderID, sizeof(ExampleOrderID) - 1);
    AMRObject_SetIMSerialNumber(block1, ExampleSerialNumber, sizeof(ExampleSerialNumber) -1 );
    ...
    AMRObject_IMAnnotation(block2, ExampleAnnotation, sizeof (ExampleAnnotation) -1 );
    AMRObject_SetIMOrderID(block2, ExampleOrderID, sizeof(ExampleOrderID) - 1);
    AMRObject_SetIMSerialNumber(block2, ExampleSerialNumber, sizeof(ExampleSerialNumber) -1);
    ...
    switch ( type )
    {
        case Asset_LocationLevelTree:
            ...
            while (src_off < num)
            {
                ...
                if(dst_off <= 13)
                {
                    dst[13 - dst_off] |= (PNIO_UINT16)(( src[src_off] & mask ) << shift );
				}
                if (numBits < 10)
                {
                    mask = ( 1 << ( 10 - numBits ) ) - 1;
