...
/*****************************************************************************/
    ...
    PNIO_UINT32 TcpTraceUpload  (PNIO_VOID)
    {
...

        {
            ...
            PNIO_INT RestBytes       = (PNIO_INT) TrcGetFillSize();
            PNIO_INT RestBytes_htonl = TRACE_HTONL(RestBytes);
            PNIO_INT BufOffset       = 0;
            PNIO_INT BytesToSend     = 0;
            PNIO_INT BytesSent       = 0;
#define HYSO_TEMP_FIX_FOR_TRACES_DOWNLOAD
#ifdef HYSO_TEMP_FIX_FOR_TRACES_DOWNLOAD
            PNIO_INT BuffersToSend = 1;
            PNIO_INT BuffersToSend_htonl = TRACE_HTONL( BuffersToSend );
